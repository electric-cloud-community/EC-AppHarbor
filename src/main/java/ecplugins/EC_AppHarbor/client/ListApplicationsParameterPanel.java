...
// ListApplicationsParameterPanel.java --
//
// ListApplicationsParameterPanel.java is part of ElectricCommander.
//
 ...
 * real function.
 */
public class ListApplicationsParameterPanel
        ...
        ParameterPanelProvider
{
    ...
    // ~ Static fields/initializers----------------------------
    private static UiBinder<Widget, ListApplicationsParameterPanel> s_binder =
    ...
    // ~ Instance fields
    // --------------------------------------------------------
    @UiField FormBuilder LAParameterForm;
    ...
    @Override public Widget doInit()
    {
        ...
        Action.addItem("List All", "list");
        Action.addItem("Get App Detail", "get");
        LAParameterForm.addRow(true, "Configuration:",
            "The name of the configuration that contains the information to connect with AppHarbor.",
            CONFIGNAME, "", new TextBox());
        LAParameterForm.addRow(true, "Action:",
            "Select the action to perform, the options are: List All and Get application detail.",
            ACTION, "list", Action);
        LAParameterForm.addRow(false, "Application Slug:",
    ...
    {
        boolean validationStatus = LAParameterForm.validate();
        String  action           = LAParameterForm.getValue(ACTION);

        if (StringUtil.isEmpty(LAParameterForm.getValue(CONFIGNAME))) {
            LAParameterForm.setErrorMessage(CONFIGNAME,
                "This Field is required.");
            validationStatus = false;
        }

        if (StringUtil.isEmpty(LAParameterForm.getValue(ACTION)
                                              .trim())) {
            LAParameterForm.setErrorMessage(ACTION, "This Field is required.");
            validationStatus = false;
        }

        if ((action.equals("get"))
                && (StringUtil.isEmpty(
                        LAParameterForm.getValue(APPSLUG)
                                   .trim()))) {
            LAParameterForm.setErrorMessage(APPSLUG, "This Field is required.");
            validationStatus = false;
        }
    ...
    private void updateRowVisibility()
    {
        ...
        if (action.equals("list")) {
            LAParameterForm.setRowVisible(APPSLUG, false);
        }
        else {
            LAParameterForm.setRowVisible(APPSLUG, true);
        }
    }
    ...
    @Override public Map<String, String> getValues()
    {
        ...
        Map<String, String> searchFormValues = LAParameterForm.getValues();
            ...
            Collection<ActualParameter> actualParameters)
    {
        ...
        for (String key : new String[] {CONFIGNAME, ACTION, APPSLUG}) {
            LAParameterForm.setValue(key,
    ...
    interface Binder
        extends UiBinder<Widget, ListApplicationsParameterPanel> { }
}
