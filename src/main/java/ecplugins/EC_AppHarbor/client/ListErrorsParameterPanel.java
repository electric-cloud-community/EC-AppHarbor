...
// ListErrorsParameterPanel.java --
//
// ListErrorsParameterPanel.java is part of ElectricCommander.
//
 ...
 * real function.
 */
public class ListErrorsParameterPanel
        ...
        ParameterPanelProvider
{
    ...
    // ~ Static fields/initializers----------------------------
    private static UiBinder<Widget, ListErrorsParameterPanel> s_binder = GWT
            .create(Binder.class);
    ...
    // ~ Instance fields
    // --------------------------------------------------------
    @UiField FormBuilder LEParameterForm;
    ...
    @Override public Widget doInit()
    {
        ...
        Action.addItem("List Error Logs", "list");
        Action.addItem("Get Error Detail", "get");
        LEParameterForm.addRow(true, "Configuration:",
            "The name of the configuration that contains the information to connect with AppHarbor.",
            CONFIGNAME, "", new TextBox());
        LEParameterForm.addRow(true, "Action:",
            "Select the Action to perform, the options are: List Error Logs and Get Error Detail.",
            ACTION, "list", Action);
        LEParameterForm.addRow(true, "Application Slug:",
            "The globally unique, URL-friendly version of the application name.",
            APPSLUG, "", new TextBox());
        LEParameterForm.addRow(false, "Error:", "The Error identifier.", ERROR,
            "", new TextBox());
    ...
    {
        boolean validationStatus = LEParameterForm.validate();
        String  action           = LEParameterForm.getValue(ACTION);

        if (StringUtil.isEmpty(LEParameterForm.getValue(CONFIGNAME))) {
            LEParameterForm.setErrorMessage(CONFIGNAME,
                "This Field is required.");
            validationStatus = false;
        }

        if (StringUtil.isEmpty(LEParameterForm.getValue(ACTION)
                                              .trim())) {
            LEParameterForm.setErrorMessage(ACTION, "This Field is required.");
            validationStatus = false;
        }

        if (StringUtil.isEmpty(LEParameterForm.getValue(APPSLUG)
                                              .trim())) {
            LEParameterForm.setErrorMessage(APPSLUG, "This Field is required.");
            validationStatus = false;
        }

        if ((action.equals("get"))
                && (StringUtil.isEmpty(
                        LEParameterForm.getValue(ERROR)
                                   .trim()))) {
            LEParameterForm.setErrorMessage(ERROR, "This Field is required.");
            validationStatus = false;
        }
    ...
    private void updateRowVisibility()
    {
        ...
        LEParameterForm.setRowVisible(ERROR, "get".equals(action));
    }
    ...
    @Override public Map<String, String> getValues()
    {
        ...
        Map<String, String> searchFormValues = LEParameterForm.getValues();
        ...
        actualParams.put(ERROR, searchFormValues.get(ERROR));
            ...
            Collection<ActualParameter> actualParameters)
    {
        ...
        for (String key : new String[] {CONFIGNAME, ACTION, APPSLUG, ERROR}) {
            LEParameterForm.setValue(key,
    ...
    interface Binder
        extends UiBinder<Widget, ListErrorsParameterPanel> { }
}
