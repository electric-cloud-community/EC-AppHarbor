...
// ListServiceHooksParameterPanel.java --
//
// ListServiceHooksParameterPanel.java is part of ElectricCommander.
//
 ...
 * real function.
 */
public class ListServiceHooksParameterPanel
        ...
        ParameterPanelProvider
{
    ...
    // ~ Static fields/initializers----------------------------
    private static UiBinder<Widget, ListServiceHooksParameterPanel> s_binder =
    ...
    static final String SERVICEHOOK = "ServiceHook";

    //~ Instance fields --------------------------------------------------------

    // ~ Instance fields
    // --------------------------------------------------------
    @UiField FormBuilder LSHParameterForm;
    ...
    @Override public Widget doInit()
    {
        ...
        Action.addItem("List Service Hooks", "list");
        Action.addItem("Get Service Hook Detail", "get");
        LSHParameterForm.addRow(true, "Configuration:",
            "The name of the configuration that contains the information to connect with AppHarbor.",
            CONFIGNAME, "", new TextBox());
        LSHParameterForm.addRow(true, "Action:",
            "Select the Action to perform, the options are: List Service Hooks and Get Service Hook Detail.",
            ACTION, "list", Action);
        LSHParameterForm.addRow(true, "Application Slug:",
            "The globally unique, URL-friendly version of the application name.",
            APPSLUG, "", new TextBox());
        LSHParameterForm.addRow(false, "Service Hook:",
            "Specifies the Service Hook identifier.", SERVICEHOOK, "",
            new TextBox());
    ...
    {
        boolean validationStatus = LSHParameterForm.validate();
        String  action           = LSHParameterForm.getValue(ACTION);

        if (StringUtil.isEmpty(LSHParameterForm.getValue(CONFIGNAME))) {
            LSHParameterForm.setErrorMessage(CONFIGNAME,
                "This Field is required.");
            validationStatus = false;
        }

        if (StringUtil.isEmpty(LSHParameterForm.getValue(ACTION)
                                               .trim())) {
            LSHParameterForm.setErrorMessage(ACTION, "This Field is required.");
            validationStatus = false;
        }

        if (StringUtil.isEmpty(LSHParameterForm.getValue(APPSLUG)
                                               .trim())) {
            LSHParameterForm.setErrorMessage(APPSLUG,
                "This Field is required.");
            validationStatus = false;
        }
        ...
        if ((action.equals("get"))
                && (StringUtil.isEmpty(
                        LSHParameterForm.getValue(SERVICEHOOK)
                                    .trim()))) {
            LSHParameterForm.setErrorMessage(SERVICEHOOK,
                "This Field is required.");
            validationStatus = false;
        }
    ...
    private void updateRowVisibility()
    {
        ...
        LSHParameterForm.setRowVisible(SERVICEHOOK, "get".equals(action));
    }
    ...
    @Override public Map<String, String> getValues()
    {
        ...
        Map<String, String> searchFormValues = LSHParameterForm.getValues();
        ...
        actualParams.put(SERVICEHOOK, searchFormValues.get(SERVICEHOOK));
    ...
    interface Binder
        extends UiBinder<Widget, ListServiceHooksParameterPanel> { }
}
