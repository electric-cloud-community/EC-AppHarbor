...
// BuildsActionsParameterPanel.java --
//
// BuildsActionsParameterPanel.java is part of ElectricCommander.
//
 ...
 * real function.
 */
public class BuildsActionsParameterPanel
        ...
        ParameterPanelProvider
{
    ...
    // ~ Static fields/initializers----------------------------
    private static UiBinder<Widget, BuildsActionsParameterPanel> s_binder = GWT
            .create(Binder.class);
    ...
    // ~ Instance fields
    // --------------------------------------------------------
    @UiField FormBuilder BAParameterForm;
    ...
    @Override public Widget doInit()
    {
        ...
        Action.addItem("List Build History", "listbuild");
        Action.addItem("Get Build Detail", "getbuild");
        Action.addItem("List Tests", "listtest");
        Action.addItem("Get Test Detail", "gettest");
        BAParameterForm.addRow(true, "Configuration:",
            "The name of the configuration that contains the information to connect with AppHarbor.",
            CONFIGNAME, "", new TextBox());
        BAParameterForm.addRow(true, "Action:",
            "Select the Action to perform, the options are: List Build History, Get Build Detail, List Tests and Get Test Detail.",
            ACTION, "listbuild", Action);
        BAParameterForm.addRow(true, "Application Slug:",
            "The globally unique, URL-friendly version of the application name.",
            APPSLUG, "", new TextBox());
        BAParameterForm.addRow(false, "Build:",
            "Specifies the build identifier.", BUILD, "", new TextBox());
        BAParameterForm.addRow(false, "Test:", "Specifies the test identifier.",
            TEST, "", new TextBox());
    ...
    {
        boolean validationStatus = BAParameterForm.validate();
        String  action           = BAParameterForm.getValue(ACTION);

        if (StringUtil.isEmpty(BAParameterForm.getValue(CONFIGNAME))) {
            BAParameterForm.setErrorMessage(CONFIGNAME,
                "This Field is required.");
            validationStatus = false;
        }

        if (StringUtil.isEmpty(BAParameterForm.getValue(ACTION)
                                              .trim())) {
            BAParameterForm.setErrorMessage(ACTION, "This Field is required.");
            validationStatus = false;
        }

        if (StringUtil.isEmpty(BAParameterForm.getValue(APPSLUG)
                                              .trim())) {
            BAParameterForm.setErrorMessage(APPSLUG, "This Field is required.");
            validationStatus = false;
        }

        if ((action.equals("getbuild"))
                && (StringUtil.isEmpty(
                        BAParameterForm.getValue(BUILD)
                                   .trim()))) {
            BAParameterForm.setErrorMessage(BUILD, "This Field is required.");
            validationStatus = false;
        }

        if ((action.equals("gettest"))
                && (StringUtil.isEmpty(BAParameterForm.getValue(TEST)
                                                      .trim()))) {
            BAParameterForm.setErrorMessage(TEST, "This Field is required.");
            validationStatus = false;
        }
    ...
    private void updateRowVisibility()
    {
        ...
        BAParameterForm.setRowVisible(BUILD, "getbuild".equals(action));
        BAParameterForm.setRowVisible(TEST, "gettest".equals(action));
    }
    ...
    @Override public Map<String, String> getValues()
    {
        ...
        Map<String, String> searchFormValues = BAParameterForm.getValues();
        ...
        actualParams.put(BUILD, searchFormValues.get(BUILD));
        actualParams.put(TEST, searchFormValues.get(TEST));
    ...
    interface Binder
        extends UiBinder<Widget, BuildsActionsParameterPanel> { }
}
