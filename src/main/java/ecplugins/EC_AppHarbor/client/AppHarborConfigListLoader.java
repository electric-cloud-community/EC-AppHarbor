...
// AppHarborConfigListLoader.java --
//
// AppHarborConfigListLoader.java is part of ElectricCommander.
//
...
public class AppHarborConfigListLoader
    extends Loader
{

    //~ Instance fields --------------------------------------------------------

    private final AppHarborConfigList m_configList;
    private final CgiRequestProxy       m_cgiRequestProxy;
    private String                      m_editorName;

    //~ Constructors -----------------------------------------------------------

    public AppHarborConfigListLoader(
         AppHarborConfigList configList,
            Component             component,
    ...
    private void loadConfigs(Map<String, String> cgiParams)
    {
        ...
        try {
                    ...
                    new RequestCallback() {
                                ...
                                Throwable exception)
                        {
                            ((HasErrorPanel) m_component).addErrorMessage(
                                "Error loading AppHarbor configuration list: ",
                                exception);
                        }
            ...
            if (m_component instanceof HasErrorPanel) {
                ((HasErrorPanel) m_component).addErrorMessage(
                    "Error loading AppHarbor configuration list: ", e);
            }
            else {
                m_component.getLog()
                           .error(e);
    ...
    private void loadEditors()
    {
        ...
        request.setPropertyName("/plugins/EC-AppHarbor/project/ui_forms/"
                + m_editorName);
        request.setExpand(false);
        request.setCallback(new EditorLoaderCallback("AppHarborcfg"));
        ...
        implements PropertyCallback
    {
        ...
        @Override public void handleResponse(Property response)
        {
            ...
            String errorMsg = "Editor '" + m_editorName
                    + "' not found for AppHarbor plugin '" + m_configPlugin
                    + "'";
