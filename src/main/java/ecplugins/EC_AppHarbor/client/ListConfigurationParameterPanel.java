...
// ListConfigurationParameterPanel.java --
//
// ListConfigurationParameterPanel.java is part of ElectricCommander.
//
 ...
 * real function.
 */
public class ListConfigurationParameterPanel
        ...
        ParameterPanelProvider
{
    ...
    // ~ Static fields/initializers----------------------------
    private static UiBinder<Widget, ListConfigurationParameterPanel> s_binder =
    ...
    static final String CONFIGVAR  = "ConfigVar";

    //~ Instance fields --------------------------------------------------------

    // ~ Instance fields
    // --------------------------------------------------------
    @UiField FormBuilder LCParameterForm;
    ...
    @Override public Widget doInit()
    {
        ...
        Action.addItem("List Configuration Variables", "list");
        Action.addItem("Get Configuration Variable Detail", "get");
        LCParameterForm.addRow(true, "Configuration:",
            "The name of the configuration that contains the information to connect with AppHarbor.",
            CONFIGNAME, "", new TextBox());
        LCParameterForm.addRow(true, "Action:",
            "Select the Action to perform, the options are: List Configuration Variables and Get Configuration Variable Detail.",
            ACTION, "list", Action);
        LCParameterForm.addRow(true, "Application Slug:",
            "The globally unique, URL-friendly version of the application name.",
            APPSLUG, "", new TextBox());
        LCParameterForm.addRow(false, "Configuration Variable:",
            "Specifies the configuration variable identifier.", CONFIGVAR, "",
            new TextBox());
    ...
    {
        boolean validationStatus = LCParameterForm.validate();
        String  action           = LCParameterForm.getValue(ACTION);

        if (StringUtil.isEmpty(LCParameterForm.getValue(CONFIGNAME))) {
            LCParameterForm.setErrorMessage(CONFIGNAME,
                "This Field is required.");
            validationStatus = false;
        }

        if (StringUtil.isEmpty(LCParameterForm.getValue(ACTION)
                                              .trim())) {
            LCParameterForm.setErrorMessage(ACTION, "This Field is required.");
            validationStatus = false;
        }

        if (StringUtil.isEmpty(LCParameterForm.getValue(APPSLUG)
                                              .trim())) {
            LCParameterForm.setErrorMessage(APPSLUG, "This Field is required.");
            validationStatus = false;
        }

        if ((action.equals("get"))
                && (StringUtil.isEmpty(
                        LCParameterForm.getValue(CONFIGVAR)
                                   .trim()))) {
            LCParameterForm.setErrorMessage(CONFIGVAR,
                "This Field is required.");
            validationStatus = false;
        }
    ...
    private void updateRowVisibility()
    {
        ...
        LCParameterForm.setRowVisible(CONFIGVAR, "get".equals(action));
    }
    ...
    @Override public Map<String, String> getValues()
    {
        ...
        Map<String, String> searchFormValues = LCParameterForm.getValues();
        ...
        actualParams.put(CONFIGVAR, searchFormValues.get(CONFIGVAR));
    ...
    interface Binder
        extends UiBinder<Widget, ListConfigurationParameterPanel> { }
}
